...
class RsSdkType : SdkType(RUST_SDK_ID_NAME) {
    ...
    override fun getHomeChooserDescriptor(): FileChooserDescriptor =
        object : FileChooserDescriptor(true, true, false, false, false, false) {
            override fun validateSelectedFiles(files: Array<VirtualFile>) {
                ...
                if (!isValidSdkHome(file.path)) {
                    throw Exception("Invalid Rust toolchain name '${file.name}'")
                }
            }
        ...
        }.withTitle("Select Rust Toolchain or Rustup").withShowHiddenFiles(SystemInfo.isUnix)
    ...
    override fun createAdditionalDataConfigurable(sdkModel: SdkModel, sdkModificator: SdkModificator): AdditionalDataConfigurable =
        RsAdditionalDataConfigurable(sdkModel, sdkModificator)
